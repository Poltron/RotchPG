/*:
*
* @plugindesc Ritter's Event Spawner v1.2
*
* @author Ritter
*
* @help
...
* removeSave = false to keep the event on save list tox respawn up re-entering
...
Ritter_DataManager_onLoad = DataManager.onLoad;
DataManager.onLoad = function(object) {
	
	for (i = 0; i < Ritter.Params.preloadMaps.length; i++) {
		var map = '$SpawnMap%1'.format(Ritter.Params.preloadMaps[i]);
		if (object === window[map]) {
			if (!Ritter._SpawnMaps) Ritter._SpawnMaps = [];
	...
	Ritter_DataManager_onLoad.call(this,object);	
};
...
Ritter.spawnEvent = function(mapId, eventId, x, y, save) {
	...
	//if (Ritter._SpawnMaps.contains(mapId)) $gameMap.spawnEvent(mapId, eventId, x, y, save);
};
...
Spriteset_Map.prototype.createCharacters = function() {
    if ($gameMap._spawnedList.length > 0) {
		if ($gameMap.event(Ritter.Params.FirstId)._eventId === Ritter.Params.FirstId && $dataMap.events[Ritter.Params.FirstId] == undefined) Ritter.recreateDatamapEvents();
	}
